...
import com.sap.iot.ain.notification.context.NotificationObjectHandler;
 ...
 *
 */
public class NotificationPOSTValidator
		implements ConstraintValidator<NotificationPOSTValiadtions, NotificationPOST> {

	private static final Logger logger = LoggerFactory.getLogger(NotificationPOSTValidator.class);

	@Autowired
	private NotificationDao notificationDao;


	public void initialize(NotificationPOSTValiadtions constraintAnnotation) {}

	public boolean isValid(NotificationPOST notification, ConstraintValidatorContext context) {
		...
		logger.debug("outside AlertID validation" + notification.getAlertIDs());
		if (!ObjectUtils.isListEmpty(notification.getAlertIDs())) {
			logger.debug("Alert ID validation");
			List<Object> alertIDs = notification.getAlertIDs().stream()
					.filter(alertID -> alertID != null && alertID.getId() != null && !alertID.getId().isEmpty())
					.collect(Collectors.toList());
			List<Object> duplicateElements = ObjectUtils.getDuplicateElements(alertIDs);

			if (!duplicateElements.isEmpty()) {
				context.buildConstraintViolationWithTemplate("notification.duplicate.alertIDs")
						.addConstraintViolation();
				return false;
			}
//			Set<String> payloadAlertIds =
//					alertIDs.stream().map(alert -> alert.getId()).collect(Collectors.toSet());
//			if(payloadAlertIds != null && !payloadAlertIds.isEmpty()) {
//			    List<NotificationAlertMapping> dbNotificationIdsForAlerts = notificationDao.getNotificationsForAlert(payloadAlertIds);
//			    if (dbNotificationIdsForAlerts != null && !dbNotificationIdsForAlerts.isEmpty()) {
//					context.buildConstraintViolationWithTemplate("notification.invalid.alertIDs")
//							.addConstraintViolation();
//					return false;
//				}
//			}			
		...
		notificationContext = cacheManager.getObject(new NotificationObjectHandler(notification));
		...
		/* Equipment and Location validation */
		...
		if(!notificationContext.isCauseValid()) {
			
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("invalid.notification.causeID")).addConstraintViolation();
			return false;
		}
		
		if(!notificationContext.isProposeFailureModeValid()) {
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("invalid.notification.ProposedFailureModeID")).addConstraintViolation();
			return false;
		}
		if(!notificationContext.isSystemProposedFailureModeValid()) {
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("invalid.notification.SystemProposedFailureMode")).addConstraintViolation();
			return false;
		}
		if(!notificationContext.isConfirmedFailureModeValid()) {
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("invalid.notification.ConfirmedFailureMode")).addConstraintViolation();
			return false;
		}
		if(!notificationContext.isInstructionValid()) {
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("instruction.id.invalid")).addConstraintViolation();
			return false;
		}
		if(!notificationContext.isEffectValid()) {
			context.buildConstraintViolationWithTemplate(
					MsgHelper.buildMsg("invalid.notification.effcetID")).addConstraintViolation();
			return false;
		}
		...
		if (failureModeIds.size() > 0) {
			...
			List<Authorization> invalidAuthorizations = authorizationList.stream()
	...
	private void trimNotificationFields(NotificationPOST notification) {
		try {
			...
			logger.debug("Excetion occured while trimming notification post payload fields...");
